...
import { ArrowRight, MapPin, Building2, X, MessageCircle, Send, Check } from "lucide-react";
...
export default function Properties() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Construct the URL immediately to be ready for the browser gesture
    ...
    // Shortened delay (500ms) to ensure mobile browsers don't block the redirect
    setTimeout(() => {
  ...
  return (
    <section className="bg-[#FAF9F6] py-0 relative overflow-visible">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        {/* HEADER SECTION */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8 pt-20">
                ...
                <div className="p-6 md:p-8 flex flex-col flex-grow">
                  ...
                  <div className="flex justify-between items-end pt-5 border-t border-gray-50">
                    <div className="flex flex-col">
                      ...
                      <p className="font-bold text-[#0B1C3D] text-lg italic leading-none">{item.price}</p>
                    </div>
                    <button 
                      onClick={() => setSelectedProp(item)}
                      className="bg-white text-[#0B1C3D] border-2 border-[#F5B300] px-4 py-2 rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-[#0B1C3D] hover:border-[#0B1C3D] hover:text-[#F5B300] transition-all flex items-center gap-2"
                    >
      ...
      <AnimatePresence>
        {selectedProp && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-[#0B1C3D]/80 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl relative">
              <button onClick={() => setSelectedProp(null)} className="absolute top-6 right-6 p-2 text-gray-400 hover:text-[#0B1C3D] transition-colors"><X size={24} /></button>
              ...
              <div className="p-8 md:p-12">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <input required type="text" placeholder="Full Name" value={formData.name} className="w-full bg-[#FAF9F6] border border-gray-100 p-4 rounded-xl text-sm outline-none focus:border-[#F5B300] text-[#0B1C3D]" onChange={(e) => setFormData({...formData, name: e.target.value})} />
                  <input required type="tel" placeholder="Mobile Number" value={formData.phone} className="w-full bg-[#FAF9F6] border border-gray-100 p-4 rounded-xl text-sm outline-none focus:border-[#F5B300] text-[#0B1C3D]" onChange={(e) => setFormData({...formData, phone: e.target.value})} />
                  <input required type="email" placeholder="Email Address" value={formData.email} className="w-full bg-[#FAF9F6] border border-gray-100 p-4 rounded-xl text-sm outline-none focus:border-[#F5B300] text-[#0B1C3D]" onChange={(e) => setFormData({...formData, email: e.target.value})} />
                  ...
                  <button 
                    ...
                    className={`w-full py-4 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${
                      isSuccess ? "bg-green-600 text-white" : "bg-[#0B1C3D] text-[#F5B300]"
                    }`}
                  >
                    {isSubmitting ? <Send size={16} className="animate-pulse" /> : isSuccess ? <Check size={18} /> : "Secure Details"}
                  </button>
                </form>

                <div className="mt-8 flex flex-col items-center gap-5">
                  <div className="w-full h-[1px] bg-gray-100 relative">
                    <span className="absolute left-1/2 -top-2 -translate-x-1/2 bg-white px-3 text-[9px] text-gray-400 uppercase tracking-widest font-bold">OR</span>
                  </div>
                  <a href="https://wa.me/919899152327" target="_blank" rel="noopener noreferrer" className="w-full bg-[#25D366] text-white px-6 py-4 rounded-xl flex items-center justify-center gap-3 shadow-lg">
                    <MessageCircle size={20} fill="currentColor" stroke="none" />
                    <span className="font-bold text-sm uppercase tracking-wider">WhatsApp Advisor</span>
                  </a>
                </div>
              </div>
            </motion.div>
